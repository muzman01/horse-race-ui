...
interface MarketItemModalProps {
  title: string;
  price: number; // TON olarak fiyat
}

const MarketItemModal2CustomTon: React.FC<MarketItemModalProps> = ({
  title,
  price,
}) => {
        ...
        <div className="p-8 bg-[#1e1e1e] rounded-2xl min-h-[250px] flex flex-col justify-between">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              onFocus={(e) => e.target.select()} // Inputa odaklanıldığında değeri otomatik seç
              className="bg-[#2b2f36] text-white p-2 rounded-lg ml-2 w-20 text-center"
              min="1"
              step="1"
            />
          </div>
...
export default MarketItemModal2CustomTon;
